...
namespace {
  ...
  const Score RookOnFile[2] = { S(25, 7), S(55, 20) };
  ...
  const Score ThreatByKing[2] = { S(30, 62), S(50, 138) };
  ...
  Score evaluate_pieces(const Position& pos, EvalInfo& ei, Score* mobility,
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT || Pt == ROOK)
        {
            ...
            if (bb & s)
                score += Outpost[Pt == BISHOP ? 1 : Pt == KNIGHT ? 0 : 2][!!(ei.attackedBy[Us][PAWN] & s)];
            else
            {
                bb &= b & ~pos.pieces(Us);
                if (bb)
                   score += ReachableOutpost[Pt == BISHOP ? 1 : Pt == KNIGHT ? 0 : 2][!!(ei.attackedBy[Us][PAWN] & bb)];
            }
...
Value Eval::evaluate(const Position& pos) {
  ...
  //if (pos.piece_on(SQ_C3) == W_KING && pos.side_to_move() == WHITE) {
	//  score += WIN;
  //}
